...
Строка читается с конца к началу. Очень удобно и лаконично.
...
Python начнет с индекса 4 и пойдет влево до индекса 2 включительно. Индекс 1 в результат не входит.

## Срез с переменными

Срезы не обязаны быть жестко зафиксированы числами. Можно использовать переменные:
...
value[:]        # Hexlet
value[::2]      # Hxe     — четные символы
value[1::2]     # elt     — нечетные символы
