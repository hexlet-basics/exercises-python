...
```text
def repeat(text, times=2):    ← times имеет значение по умолчанию
    ...
...
Необязательные параметры всегда указываются в конце списка параметров. Поэтому в примере сначала идет обязательный параметр `text`, а уже после него располагается параметр `times` со значением по умолчанию.
