...
Но можно записать более лаконично. Для этого справа от `return` должно быть выражение, однако `if` является инструкцией, которую нельзя использовать как выражение. В Python есть конструкция, которая работает как `if-else`, но считается выражением. Она называется **тернарный оператор** и является единственным оператором в Python, который требует три операнда:
