...
У переменной `sum` есть начальное значение, с которого начинается любая повторяющаяся операция. В примере выше это `0`.

В математике есть понятие **нейтральный элемент**, и у каждой операции он свой. Операция с этим элементом не изменяет то значение, над которым работает. Например, в сложении любое число плюс ноль дает само число. При вычитании нейтральный элемент тот же: 0. У конкатенации нейтральным элементом является пустая строка: `'' + 'one'` будет 'one'.
